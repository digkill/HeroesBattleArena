...
func create_icon_for_unit(unit: Unit) -> ColorRect:
	...
	var icon_dim: float = hero_icon_size if unit is Hero else icon_size
	icon.size = Vector2(icon_dim, icon_dim)
...
func update_icon_positions() -> void:
	var texture_size: Vector2 = minimap_texture.size
...
func world_to_minimap(world_pos: Vector3) -> Vector2:
	...
	var x: float = (1.0 - norm_x) * texture_size.x if invert_x else norm_x * texture_size.x
	var y: float = (1.0 - norm_y) * texture_size.y if invert_y else norm_y * texture_size.y
	return Vector2(x, y)
...
func minimap_to_world(minimap_pos: Vector2) -> Vector3:
	...
	if texture_size.x <= 0.0 or texture_size.y <= 0.0:
		return Vector3.ZERO
	var norm_x: float = clamp(minimap_pos.x / texture_size.x, 0.0, 1.0)
	var norm_y: float = clamp(minimap_pos.y / texture_size.y, 0.0, 1.0)
	if invert_x:
		norm_x = 1.0 - norm_x
	if invert_y:
		norm_y = 1.0 - norm_y
