...
func _physics_process(delta: float) -> void:
	...
	# Обновляем настройки NavigationAgent3D
	if nav_agent != null:
		nav_agent.max_speed = move_speed
		# NavigationAgent3D автоматически синхронизируется с родителем (CharacterBody3D)
...
func setup_navigation_agent() -> void:
	...
	nav_agent.max_speed = move_speed
	# NavigationAgent3D автоматически синхронизируется с родителем
	# Ждем один кадр, чтобы навигация была готова
	call_deferred("_ensure_nav_ready")
...
func _ensure_nav_ready() -> void:
	# Убеждаемся, что навигация готова к использованию
	if nav_agent != null:
		# NavigationAgent3D автоматически синхронизируется с родителем
		pass

func find_nearest_enemy_in_range(range_dist: float) -> Unit:
	var nearest: Unit
	var best_dist := range_dist
